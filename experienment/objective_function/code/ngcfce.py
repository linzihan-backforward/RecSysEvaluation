...
class NGCFCE(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(NGCFCE, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        pos_scores = self.sigmoid(pos_scores)
        mf_loss = self.mf_loss(pos_scores, label)  # calculate BPR Loss

        reg_loss = self.reg_loss(u_embeddings, pos_embeddings)  # L2 regularization of embeddings
