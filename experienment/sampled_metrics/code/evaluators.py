...
class TopKEvaluator(GroupedEvaluator):
    ...
    def collect(self, interaction, scores_tensor):
       ...
       """
        ...
        shape_matrix = torch.full((len(user_len_list), 1), scores_matrix.shape[1],
                                  device=scores_matrix.device, dtype=torch.long)
    ...
    def evaluate(self, batch_matrix_list, eval_data):
        ...
        batch_result = torch.cat(batch_matrix_list, dim=0)
        # unpack top_idx and shape_matrix
        topk_idx = batch_result[:, :-1]
        shapes = batch_result[:, -1]

        pos_idx_matrix = (topk_idx >= (shapes - torch.tensor(pos_len_list, device=topk_idx.device)).reshape(-1, 1))
    ...
    def metrics_info(self, pos_idx_matrix, pos_len_list, metrics):
        metric_dict = {}
        pos_idx_matrix = pos_idx_matrix.cpu().numpy()
    ...
    def scale_up_rank(self, rank, user_len_list, tot_item_num):
        unbias_pos_idx = torch.full_like(rank, False)
        pos_idx_rows, pos_idx_cols = torch.nonzero(rank, as_tuple=True)
        user_len_list = torch.tensor(user_len_list, device=rank.device)
        user_len = user_len_list[pos_idx_rows]
        unbias_rank = (pos_idx_cols * (tot_item_num - 1)) // user_len
        unbias_topk_idx = torch.where(unbias_rank < max(self.topk))[0]
        pos_idx_rows, unbias_rank = pos_idx_rows[unbias_topk_idx], unbias_rank[unbias_topk_idx]
        unbias_pos_idx[pos_idx_rows, unbias_rank] = True

        return unbias_pos_idx
    ...
    def _calculate_metrics(self, pos_len_list, pos_idx_matrix):
...
class RankEvaluator(GroupedEvaluator):
    ...
    def evaluate(self, batch_matrix_list, eval_data):
        ...
        pos_len_list = eval_data.get_pos_len_list()
        user_len_list = eval_data.get_user_len_list()
        pos_rank_sum = torch.cat(batch_matrix_list, dim=0)
        assert len(pos_len_list) == len(pos_rank_sum)

        # get metrics
        metric_dict = self.metrics_info(user_len_list, pos_len_list, pos_rank_sum, self.metrics)

        # get unbias result
        if self.unbias:
            metrics = ['unbias_' + metric for metric in self.metrics]
            unbias_rank_sum = self.scale_up_rank(pos_rank_sum, user_len_list, eval_data.item_num)
            metric_dict.update(
                self.metrics_info(np.full_like(user_len_list, eval_data.item_num),
                                  pos_len_list, unbias_rank_sum, metrics))

        return metric_dict

    def metrics_info(self, user_len_list, pos_len_list, pos_rank_sum, metrics):
        metric_dict = {}
        pos_rank_sum = pos_rank_sum.cpu().numpy()
    ...
    def scale_up_rank(self, rank, user_len_list, tot_item_num):
        weight = torch.tensor((tot_item_num - 1) / (user_len_list - 1), device=rank.device)
        unbias_rank = torch.floor(rank * weight.unsqueeze(1)) + 1
        return unbias_rank
...
class UnbiasTopkEvaluator(GroupedEvaluator):
    r"""TopK Evaluator is mainly used in ranking tasks. Now, we support six topk metrics which
       contain `'Hit', 'Recall', 'MRR', 'Precision',  'NDCG', 'MAP'`.
    ...
    def collect(self, interaction, scores_tensor, item_num):
        """collect the topk intermediate result of one batch, this function mainly
        implements padding and TopK finding. It is called at the end of each batch
       ...
       """
        ...
        _, desc_index = torch.sort(scores_matrix, dim=-1, descending=True)
        pos_idx_matrix = (
                    desc_index >= (shape_matrix - torch.tensor(pos_len_list, device=desc_index.device).reshape(-1, 1)))

        # metrics = ['unbias_' + metric for metric in self.metrics]
        unbias_pos_idx = self.scale_up_rank(pos_idx_matrix, user_len_list, item_num)
        pos_idx = unbias_pos_idx[:, :max(self.topk)]
    ...
    def evaluate(self, batch_matrix_list, eval_data):
        ...
        # unpack top_idx and shape_matrix
        pos_idx_matrix = torch.cat(batch_matrix_list, dim=0)
        assert len(pos_len_list) == len(pos_idx_matrix)
...
metric_eval_bind = [
    ...
    (topk_metrics, UnbiasTopkEvaluator)
]
