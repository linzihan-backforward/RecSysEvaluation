...
from recbole.evaluator.evaluators import metric_eval_bind, group_metrics, individual_metrics, UnbiasTopkEvaluator
...
class ProxyEvaluator(object):
   ...
   """
    ...
    def __init__(self, config):
        ...
        self.unbias = self.config['unbias']
        self._check_args()
    ...
    def build(self):
        ...
        for metrics, evaluator in metric_eval_bind:
            ...
            if used_metrics:
                if evaluator == UnbiasTopkEvaluator and not self.unbias:
                    continue
                evaluator_list.append(evaluator(self.config, used_metrics))
        return evaluator_list

    def collect(self, interaction, scores, item_num):
        ...
        for evaluator in self.evaluators:
            if isinstance(evaluator, UnbiasTopkEvaluator):
                results.append(evaluator.collect(interaction, scores, item_num))
            else:
